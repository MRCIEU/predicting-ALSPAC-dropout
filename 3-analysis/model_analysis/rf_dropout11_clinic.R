...
model_clinic <- train(formula, data=dropout_11_clean, method="rf", metric=metric, trControl=control)
...
# Save the model
saveRDS(model_clinic, file="3-analysis/RData_files/model_clinic.rds")

# Plotting the ROC curve
prob_list <- model_clinic$pred$Dropout
true_list <- model_clinic$pred$obs 
fold_indices <- split(seq_along(model_clinic$pred$Resample), model_clinic$pred$Resample)
...
for (i in seq_along(fold_indices)) {
    ...
    print(summary(dropout_11_clean[indices, "clinic_dropout"])) # Replace with the column name for the dropout outcome
